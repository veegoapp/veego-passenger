...
export function BikeMap({ phase, destination }: BikeMapProps) {
  const pulse = useRef(new Animated.Value(1)).current;
  const ring1 = useRef(new Animated.Value(0)).current;
  const ring2 = useRef(new Animated.Value(0)).current;
  const bikeX = useRef(new Animated.Value(0.6)).current;
  const bikeY = useRef(new Animated.Value(0.4)).current;
  ...
  useEffect(() => {
    ...
    const m = Animated.loop(Animated.sequence([
      Animated.timing(bikeX, { toValue: 0.52, duration: 3800, useNativeDriver: false }),
      Animated.timing(bikeY, { toValue: 0.47, duration: 3200, useNativeDriver: false }),
      Animated.timing(bikeX, { toValue: 0.67, duration: 2900, useNativeDriver: false }),
      Animated.timing(bikeY, { toValue: 0.36, duration: 3600, useNativeDriver: false }),
      Animated.timing(bikeX, { toValue: 0.6, duration: 3800, useNativeDriver: false }),
      Animated.timing(bikeY, { toValue: 0.4, duration: 2800, useNativeDriver: false }),
    ]));
    m.start();
  ...
  return (
    <View style={StyleSheet.absoluteFillObject}>
      ...
      <Animated.View style={{
        position: 'absolute',
        left: bikeX.interpolate({ inputRange: [0, 1], outputRange: ['5%', '92%'] }),
        top: bikeY.interpolate({ inputRange: [0, 1], outputRange: ['10%', '85%'] }),
