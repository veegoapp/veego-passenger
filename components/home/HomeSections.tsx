...
export function ServiceCards({ c, t, isRTL, getService, isServiceVisibleForZone, onServicePress }: {
  c: ThemeColors; t: T; isRTL: boolean;
  getService: (id: ServiceType) => any;
  isServiceVisibleForZone: (id: ServiceType) => boolean;
  onServicePress: (id: string) => void;
}) {
  ...
  // The root layout mirrors row layout via the `direction` style keyed off
  // the app's `isRTL` (language === 'ar'), not the native I18nManager.isRTL
  // (which only applies after a reload). Reversing the array off the same
  // `isRTL` the mirror actually uses keeps the visual order left-to-right
  // in both languages.
  const orderedCardItems = isRTL ? [...CARD_ITEMS].reverse() : CARD_ITEMS;
