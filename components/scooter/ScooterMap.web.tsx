...
import { SCOOTER_DESTINATIONS, SCOOTER_USER_LOCATION } from './scooterMapData';
...
const SCOOTER_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="38" height="46" viewBox="0 0 38 46">
...
export function ScooterMap({ destination }: ScooterMapProps) {
  const pickupCoords: [number, number] = [
    SCOOTER_USER_LOCATION.longitude,
    SCOOTER_USER_LOCATION.latitude,
  ];

  const destEntry = destination ? SCOOTER_DESTINATIONS[destination] : null;
  const destCoords: [number, number] | null = destEntry
    ? [destEntry.longitude, destEntry.latitude]
    : null;

  const mockScooterCoords: [number, number] = [
    SCOOTER_USER_LOCATION.longitude + 0.004,
    SCOOTER_USER_LOCATION.latitude + 0.003,
  ];
  ...
  const routePoints: [number, number][] = destCoords
    ? [mockScooterCoords, pickupCoords, destCoords]
    : [mockScooterCoords, pickupCoords];
